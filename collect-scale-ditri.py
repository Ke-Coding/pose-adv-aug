...
from data.collect_scale_distri import MPII
...
def collect_data(data_loader, dataset, hg, save_path):
    ...
    for i, (img_list, heatmap_list, center_list, scale_list, rot_list,
            grnd_pts_list, normalizer_list, scale_idx, img_index, pts_aug_list)\
            in enumerate(data_loader):
        print( '%d/%d' % (i, len(data_loader)))
        # pts_aug_back = Evaluation.transform_preds(pts_aug_list[5][0]+1, center_list[5][0],
        #                         scale_list[5][0], [64, 64], rot_list[5][0])
        # print('grnd_pts: ', grnd_pts_list[5][0])
        # print('pts_aug_back: ', pts_aug_back)
        # exit()
        # print('scale_idx size ', scale_idx.size())
        # exit()
        # print(img_index)
        ...
        # print(rotation_ind)
        # exit()
        unique_num = scale_idx.size(0)
        assert scale_idx.size(1) == scale_num
        ...
        for j in range(0, unique_num):
            ...
            # exit()
            assert (tmp_pckhs.size(0) == scale_num)
            if tmp_pckhs.sum() == 0:
                print( 'tmp_pckh: ', tmp_pckhs)
