...
class MPII(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        scale_idx = torch.zeros(self.num).long()
        for i in range(0, len(self.scale_means)):
            s_tmp = s * (2 ** (self.scale_means[i]))
            scale_idx[i] = i
            scales[i] = s_tmp.clone()
            rotations[i] = r.clone()
            centers[i] = c.clone()
            origin_pts[i] = pts.clone()
            normalizers[i] = normalizer
            imgs[i], heatmaps[i], pts_augs[i] = self.gen_img_heatmap(c.clone(), s_tmp.clone(), 0,
                                                        img.clone(), pts.clone())
        # exit()
        return imgs, heatmaps, centers, scales, rotations, \
               origin_pts, normalizers, scale_idx, index, pts_augs
