...
from pylib import HumanPts, HumanAug, Evaluation
...
class MPII(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        pts_aug[idx, :] = 0
        # pts_aug_back = Evaluation.TransformPts(pts_aug+1, c.numpy(),
        #                                 s.numpy(), r, self.out_res, self.std_size, invert=1)
        # Generate ground truth
        heatmap, pts_aug = HumanPts.pts2heatmap(pts_aug, [self.out_res, self.out_res], sigma=1)
        heatmap = torch.from_numpy(heatmap).float()
        # pts_aug_back = torch.from_numpy(pts_aug_back).float()
